...
class QuoraDataLoader(DataLoader):
    def __init__(self, args):
        ...
        texts = []
        id_to_text = {}
        id_to_text_train, train_id_mapping, train_label, train_ids = self.load_dt(args.data_path, "train")
        id_to_text_dev, dev_id_mapping, dev_label, dev_ids = self.load_dt(args.data_path, "dev", len(train_ids))
        id_to_text_test, test_id_mapping, test_label, test_ids = self.load_dt(args.data_path, "test", len(train_ids) + len(dev_ids))
    ...
    def load_dt(path, flavor, offset=0):
        ...
        for idx, line in enumerate(lines):
            ...
            id_to_text[k1] = line[1]
            id_to_text[k2] = line[2]
            id_mapping[k1] = k2 
            ids.append(str(idx + offset))
            labels[str(idx + offset)] = int(line[0])
        return id_to_text, id_mapping, labels, ids
