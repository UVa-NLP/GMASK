...
class GroupMask(nn.Module):
    ...
    def forward(self, deatten, batch, pred, flag):
        # embedding
        x = deatten.embedder(batch[0])

        if flag == 'train':
            # repeat local batch
            x = x.repeat(self.local_bsz, 1, 1)
            x = x.view(self.local_bsz, -1, self.embed_dim)
            ...
            x_prime = torch.cat((x_s[:, :self.sent1_len, :], x_s.new_ones(x_s.shape[0], batch[0][0].shape[0] - self.sent1_len,
                                 x_s.shape[2]), x_s[:, self.sent1_len:, :], x_s.new_ones(x_s.shape[0],
                                 batch[0][1].shape[0] - self.sent2_len, x_s.shape[2])), dim=1) * x

            z_margin1 = torch.sum(z_prob[:int((z_prob.shape[0] / 2)), :], dim=0)
            z_margin1_p = F.softmax(z_margin1,dim=0)
            self.z_loss1 = torch.sum(z_margin1_p * torch.log(z_margin1_p+1e-8))
            z_margin2 = torch.sum(z_prob[int((z_prob.shape[0] / 2)):, :], dim=0)
            ...
            output = deatten.mask_predict(self.local_bsz, batch[0], x_prime)
            pred_loss= loss_fn(output, pred)
            _, output = output.max(dim=1)

            return pred_loss, output, z_prob, t_prob, x_prob
            ...
            x_prime = torch.cat((x_prob[:, :self.sent1_len, :], x_prob.new_ones(x_prob.shape[0], batch[0][0].shape[0] - self.sent1_len,
                                 x_prob.shape[2]), x_prob[:, self.sent1_len:, :], x_prob.new_ones(x_prob.shape[0],
                                 batch[0][1].shape[0] - self.sent2_len, x_prob.shape[2])), dim=1) * x
        
            output = deatten.mask_predict(1, batch[0], x_prime)
            pred_loss= loss_fn(output, pred)
            _, output = output.max(dim=1)
            ...
            return pred_loss, output, z_max_mat, t_prob, x_prob
...
class WordMask(nn.Module):
    ...
    def forward(self, deatten, batch, pred, flag):
        ...
        if flag == 'train':
            ...
            x_prime = torch.cat((r_s[:, :self.sent1_len, :], r_s.new_ones(r_s.shape[0], batch[0][0].shape[0] - self.sent1_len,
                                 r_s.shape[2]), r_s[:, self.sent1_len:, :], r_s.new_ones(r_s.shape[0],
                                 batch[0][1].shape[0] - self.sent2_len, r_s.shape[2])), dim=1) * x
            ...
            x_prime = torch.cat((r_prob[:, :self.sent1_len, :], r_prob.new_ones(r_prob.shape[0], batch[0][0].shape[0] - self.sent1_len,
                                 r_prob.shape[2]), r_prob[:, self.sent1_len:, :], r_prob.new_ones(r_prob.shape[0],
                                 batch[0][1].shape[0] - self.sent2_len, r_prob.shape[2])), dim=1) * x
...
def interpret(args, model, batch, pred):
    model.eval()
    sent1_len = (batch[0][0] != 0).nonzero()[-1, 0] + 1
    sent2_len = (batch[0][1] != 0).nonzero()[-1, 0] + 1
    ...
    wmask_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(wmask_optimizer, 20)
    w_beta = 1
    w_epochs = 10
    for _ in range(w_epochs):
        wmask_optimizer.zero_grad()
        pred_loss, r_prob = wmask(model, batch, pred, 'train')
        loss = pred_loss + w_beta * wmask.r_loss
        loss.backward()
        wmask_optimizer.step()
        wmask_scheduler.step()
    _, r_prob = wmask(model, batch, pred, 'test')
    ...
    for v in x_ids[:kw]:
        if v >= sent1_len:
            sent2_r.append(batch[0][1][int(v - sent1_len)])
            sent2_r_ids.append(v)
        else:
            sent1_r.append(batch[0][0][v])
    ...
    args.gnum = min(len(sent1_r), len(sent2_r))
    args.T_name = ['T_'+ str(t+1) for t in range(args.gnum)]
    sent1_r_len = len(sent1_r)
    sent2_r_len = len(sent2_r)
    batch_r_1 = batch[0][0].new_zeros(max(sent1_r_len, sent2_r_len))
    batch_r_2 = batch[0][1].new_zeros(max(sent1_r_len, sent2_r_len))
    sent1_pad_ids = [-1] * max(sent1_r_len, sent2_r_len)
    sent2_pad_ids = [-1] * max(sent1_r_len, sent2_r_len)
    sent1_pad_ids[:sent1_r_len] = sent1_r_ids
    sent2_pad_ids[:sent2_r_len] = sent2_r_ids
    sent_pad_ids = sent1_pad_ids + sent2_pad_ids
    sent_rest_ids = [i for i in x_ids if i not in sent_pad_ids]
    if sent1_r_len < max(sent1_r_len, sent2_r_len):
        sent1_r += [0] * (max(sent1_r_len, sent2_r_len) - sent1_r_len)
    if sent2_r_len < max(sent1_r_len, sent2_r_len):
        sent2_r += [0] * (max(sent1_r_len, sent2_r_len) - sent2_r_len)
    batch_r_1 = torch.LongTensor(sent1_r).to(args.device)
    batch_r_2 = torch.LongTensor(sent2_r).to(args.device)
    batch_r_com = torch.cat((batch_r_1.unsqueeze(0), batch_r_2.unsqueeze(0)), dim = 0)
    batch_r = (batch_r_com, batch[1], batch[2])

    top_words = [args.input_words[i] for i in sent1_r_ids[:sent1_r_len]]
    top_words += [args.input_words[i] for i in sent2_r_ids[:sent2_r_len]]
    ...
    for ep in range(epochs):
        optimizer.zero_grad()
        pred_loss, _, z_prob, t_prob, x_prob = gmask(model, batch_r, pred, 'train')
        loss = alpha * pred_loss + beta * gmask.t_loss + gamma * (gmask.z_loss1 + gmask.z_loss2)
        loss.backward()
        optimizer.step()
        scheduler.step()

    _, output, z_prob, t_prob, x_prob = gmask(model, batch_r, pred, 'test')
    ...
    for i, v in enumerate(x_r_ids):
        if v >= sent1_r_len:
            x_r_ids[i] += batch_r[0][0].shape[0] - sent1_r_len
